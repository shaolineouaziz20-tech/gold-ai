...
def send_discord_alert(session_name, result):
    ...
    try:
        ...
        if response.status_code in [200, 204]:
            ...
            print(f"⚠️ فشل الإرسال إلى Discord: {response.status_code} - {response.text}")
...
scheduler.add_job(auto_session_job, "cron", hour=23, minute=50, args=["Asia Session"])
scheduler.add_job(auto_session_job, "cron", hour=6, minute=50, args=["London Session"])
scheduler.add_job(auto_session_job, "cron", hour=11, minute=50, args=["New York Session"])
...
def get_gold_price():
    """جلب سعر الذهب المباشر (XAUUSD) من مصدر سريع ومحدث لتجنب جمود السعر"""
    try:
        # استعمال مصدر مالي ديناميكي سريع لـ XAUUSD (Yahoo Finance API كمصدر بديل سريع وموثوق)
        url = "https://query1.financeapp.com/v8/finance/chart/GC=F"
        ...
        print(f"Error fetching gold price from primary: {e}")
    
    # محاولة ثانية عبر الـ API المجاني كـ Backup
    try:
        ...
        print(f"Error fetching gold price from backup: {e}")
...
def fetch_and_analyze_auto():
    news_text = fetch_today_usd_news()
    analysis_result = analyze_news_and_get_bias(news_text, session_name="Upcoming Session")
...
def analyze_manual():
    ...
    analysis_result = analyze_news_and_get_bias(news_text, session_name="Manual Analysis")
    return jsonify(analysis_result)
...
def test_discord():
    test_data = {
        "daily_bias": "BULLISH",
        "daily_confidence": 90,
        "draw_on_liquidity": "Buy-side Liquidity (فوق أقمام الجلسة السابقة)",
        "major_news_alert": {
            "has_major_news": True,
            "warning": "تنبيه تجريبي: خبر CPI قادم!",
        },
        "fundamental_summary": "هذا إشعار تجريبي للتأكد من ربط Gold AI مع سيرفر Discord بنجاح! 🚀",
    }
